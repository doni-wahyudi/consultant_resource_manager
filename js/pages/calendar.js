...
const CalendarPage = {
    ...
    /**
     * Handle date click or team member drop on timeline
     * @param {string} date - Selected date
     * @param {string} talentId - Team Member ID (if dropped)
     */
    async handleDateClick(date, talentId) {
        ...
        // Include both in_progress and upcoming activities for allocation
        ...
        if (allocatableProjects.length === 0) {
            Toast.warning('No active or upcoming activities available');
            return;
        }

        // Group activities by status for better UX
        ...
        Modal.form({
            title: `Allocate ${talent ? talent.name : 'Team Member'}`,
            fields: [
                {
                    name: 'project_id',
                    label: 'Activity',
            ...
            onSubmit: async (data) => {
                ...
                try {
                    ...
                    await ActivityLogService.log('created', 'allocation', null,
                        `${talent?.name || 'Team Member'} - ${selectedProject?.name || 'Activity'}`);
    ...
    async handleAllocationClick(allocation) {
        ...
        const projects = StateManager.getState('projects') || [];
        const talents = StateManager.getState('talents') || [];
        // Include both in_progress and upcoming activities for editing
        const allocatableProjects = projects.filter(p => p.status === 'in_progress' || p.status === 'upcoming');
            ...
            Modal.show({
                ...
                content: `
                    <div class="allocation-details">
                        <p><strong>Team Member:</strong> ${talent ? talent.name : 'Unknown'}</p>
                        <p><strong>Activity:</strong> ${project ? project.name : 'Unknown'}</p>
     ...
     * @param {Array} activeProjects - Active activities list
     * @param {Object} talent - Talent object
     */
    async editAllocation(allocation, allocatableProjects, talent) {
        ...
        Modal.form({
            title: `Edit Allocation - ${talent ? talent.name : 'Team Member'}`,
            fields: [
                {
                    name: 'project_id',
                    label: 'Activity',
        ...
        if (confirmed) {
            try {
                ...
                await ActivityLogService.log('deleted', 'allocation', allocation.id,
                    `${talent?.name || 'Team Member'} - ${project?.name || 'Activity'}`);
    ...
    /**
     * Handle activity click - show activity details popup
     * @param {Object} project - Activity object
     */
    handleProjectClick(project) {
        ...
        Modal.show({
            ...
            content: `
                <div class="project-details-popup">
                    ...
                    <div class="project-detail-section">
                        <label>Assigned Team Members (${assignedTalents.length})</label>
            ...
            footer: `
                ...
                <button class="btn btn-primary" data-action="edit">Edit Activity</button>
            `,
            size: 'md'
        });
        ...
        footer?.addEventListener('click', (e) => {
            ...
            if (action === 'close') {
                ...
                Modal.hide();
                // Navigate to activities page and open edit form
